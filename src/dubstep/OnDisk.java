...
//	static Map<Expression, Double> onDiskSumExp;
//	static Map<Expression, Double> onDiskMaxExp;
//	static Map<Expression, Double> onDiskMinExp;
	static Map<Expression,PrimitiveValue> columnExp = new HashMap<Expression,PrimitiveValue>();
//	static double onDiskMaxAggregate = Double.MIN_VALUE;
//	static double onDiskMinAggregate = Double.MAX_VALUE;
//	static int onDiskCountAggregate = 0;
	...
	public static void evaluateOnDisk() throws SQLException, IOException {
		...
		while (true) {
			...
			System.err.println("Scanner reading started at time "+t1);
			try{
				while (!(strline = scanner.nextLine()).contains(lineTerminator)) {
				strline += " ";
				System.err.println("Inside scanner "+strline);
				inputAppend.append(strline);
			}
			}
			catch(Exception e){
				System.err.println("Exception in scanner read "+e);
				System.out.println("Exception in scanner read for real "+e);
				//return;
			}
			
			double t2 = (new Timestamp(System.currentTimeMillis())).getTime();
			System.err.println("Scanner reading ended at time "+t2);
			inputAppend.append(strline);
		...
		//		System.out.println("debugging1");
				...
				//	System.err.println("OnDisk Create Table "+statement.toString());
				//	System.err.println("OnDisk Create Table Start");
				//	System.out.println("debugging2");
					...
					if(referenceMap.size()==0){
						...
						referenceList.add("NATIONKEY");
						referenceList.add("CUSTKEY");
						referenceMap.put("CUSTOMER",referenceList);
						
						Set<String> referenceList1 = new HashSet<String>();
						referenceList1.add("REGIONKEY");
						referenceList1.add("NATIONKEY");
						referenceMap.put("NATION",referenceList1);
						
						Set<String> referenceList2 = new HashSet<String>();
						referenceList2.add("SUPPKEY");
						referenceList2.add("NATIONKEY");
						referenceMap.put("SUPPLIER",referenceList2);
						
						Set<String> referenceList3 = new HashSet<String>();
						referenceList3.add("REGIONKEY");
						referenceMap.put("REGION",referenceList3);
						
						Set<String> referenceList4 = new HashSet<String>();
						File lineItem = new File("data/LINEITEM.csv");
						if(lineItem.length() < 99646424){
							referenceList4.add("SUPPKEY");
							System.err.println("OnDisk Create Table May be phase 1");
						}
						else{
							System.err.println("OnDisk Create Table May be phase 2");
						}
						referenceList4.add("ORDERKEY");
				//		referenceList4.add("PARTKEY");
						
						referenceMap.put("LINEITEM",referenceList4);
						
						Set<String> referenceList5 = new HashSet<String>();
						referenceList5.add("ORDERKEY");
						referenceList5.add("CUSTKEY");
						referenceMap.put("ORDERS",referenceList5);
					
						Set<String> referenceList6 = new HashSet<String>();
						referenceList6.add("PARTKEY");
						referenceList6.add("SUPPKEY");
						referenceMap.put("PARTSUPP",referenceList6);
						
						Set<String> referenceList7 = new HashSet<String>();
						referenceList7.add("PARTKEY");
						referenceMap.put("PART",referenceList7);
					...
					createTableCounter++;
					//indexdatamap = new LinkedHashMap<String,Integer>();
					//buildIndexes(table.getTable().getName(),table.getIndexes());
...
//				if(isOnlyWhere && filecounter < indexstart){
//					filecounter++;
//					continue;
//				}
				if (isLimitbreaks)
	...
	public static void sortJoinPairs(List<String> joinTables){
		...
		for(JoinPair jp:joinPairList){
		//	System.out.println("JoinPairList "+jp.table1+","+jp.table2+","+jp.joinCol);
		}
	}
	...
	public static boolean evaluateGroupByExp(List<Expression> expressionList){
		try{
			...
			while (expIter.hasNext()) {
				...
				if(expr instanceof Column){
					Column c = (Column)expr;
			//		System.out.println("Col : "+c.getColumnName());
			//		System.out.println("Table : "+Main.tableName);
					int ind = Main.map.get(Main.tableName).get(c.getColumnName()).firstKey();
					PrimitiveValue value = Main.valslist.get(ind);
				//	System.out.println("Value : "+value.toRawString());
	...
	/*public static void buildIndexes(String tableName, List<Index> indexes) {
		ArrayList<Integer> colIndexes = new ArrayList<Integer>();
		ArrayList<Datatypes> colDataTypes = new ArrayList<Datatypes>();
		ArrayList<Boolean> colsort = new ArrayList<Boolean>();
		for (int i = 0; i < orderElements.size(); i++) {
			String colName = orderElements.get(i);
			colIndexes.add(Main.map.get(tableName).get(colName).firstKey());
			colDataTypes.add(Main.map.get(tableName).get(colName).firstEntry().getValue());
			colsort.add(true);
		}

		precomputedorderpath = ExternalSort.DoExternalSorting("data/" + tableName + ".csv", colIndexes, colDataTypes,
				colsort,"",null);
		indexname = indexes.get(1).getColumnsNames().get(0).toUpperCase();
		colIndexes.clear();
		colIndexes.add(Main.map.get(tableName).get(indexname).firstKey());
		colDataTypes.clear();
		colDataTypes.add(Main.map.get(tableName).get(indexname).firstEntry().getValue());
		indexsortedpath = ExternalSort.DoExternalSorting("data/" + tableName + ".csv", colIndexes, colDataTypes,
				colsort,"",null);
		
		BufferedReader br ;
		try{
			br = new BufferedReader(new FileReader(indexsortedpath));
			String line ="";
			int count=1;
			while((line=br.readLine())!=null){
				String[] vals = line.split("\\|");
				String value = vals[Main.map.get(tableName).get(indexname).firstKey()];
				if(!indexdatamap.containsKey(value)){
					indexdatamap.put(value, count);
				}
				count++;	
			}
		}catch (IOException e){
	...
	public static void columnInfo(List<ColumnDefinition> columns, Map<String, TreeMap<Integer, Datatypes>> cmap, String tableName) {
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			if(referenceColumns != null){/*
				...
				if(referenceColumns.get(0).equals("REFERENCES")){
					String referredTable = referenceColumns.get(1);
					String colName = columns.get(i).getColumnName();
					toSortTableData obj1 = new toSortTableData(tableName,colName);
					toSortTableData obj2 = new toSortTableData(referredTable,colName);
					if(joinColumns.containsKey(tableName)){
						Set<String> l1 = joinColumns.get(tableName);
						l1.add(colName);
						joinColumns.put(tableName,l1);	
					}
					else{
					...
					if(joinColumns.containsKey(referredTable)){
						Set<String> l2 = joinColumns.get(referredTable);
						l2.add(colName);
						joinColumns.put(referredTable,l2);
					}
					else{
		...
		//	if (columns.get(i).getColumnName().equals("RETURNFLAG")
		//			|| columns.get(i).getColumnName().equals("LINESTATUS"))
		//		orderElements.add(columns.get(i).getColumnName());
		//	InMemory.coldatatypes.add(enumDataType);
		}
	...
	/*public static void groupByOrderBy(List<Column> groupByExpr, List<Expression> expressionList, String res)
		...
		for (Column col : groupByExpr) {
			String colName = col.getColumnName();
			PrimitiveValue value = Main.valslist.get(Main.map.get(Main.tableName).get(colName).firstKey());
			if (value instanceof LongValue) {
				LongValue lv = (LongValue) value;
				res += lv.toString() + "|";
			} else if (value instanceof DoubleValue) {
		...
		if (!Main.groupByKeys.contains(res)) {
			if (Main.groupByKeys.size() > 0) {
				Main.orderByappend.append(Main.groupByKeys.first());
				Main.printResultsOrderBy(expressionList);
				if (!OnDisk.isNotPrecomputedorder) {
					Main.limitcounter++;
					System.out.print(Main.orderByappend);
					Main.orderByappend.delete(0, Main.orderByappend.length());
					if (Main.limitcounter == Main.rowCount) {
						isLimitbreaks = true;
						return;
					}
				...
				Main.minExp.clear();
				Main.countAggregate = 0;
				Main.groupByKeys.clear();
				Main.chunkCounter++;
			}
			Main.groupByKeys.add(res);
		}
		Main.countAggregate++;
		Iterator<Expression> expIter = expressionList.iterator();
